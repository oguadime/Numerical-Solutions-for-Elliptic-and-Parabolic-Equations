function FullyImplicitSystem1b(alpha, c, tau)
    if nargin < 3
        alpha = 1;
        c = 1;
        tau = 0.025;
    ...
    while t < T
        ...
        while iter < maxiter
            F = unext - uold - tau * g(t + tau) + tau * alpha * unext^3 + c * tau * (unext - ((c * tau * unext) + vold) / (1 + c * tau));
            F_prime = 3 * tau * alpha * unext^2 + (1 + 2 * c * tau) / (1 + c * tau);
